export generate_MGA_Rand_Vec!

"""
    result = generate_MGA_Rand_Vec!(
      model::JuMP.Model,
...
Generate `n_alternatives` solutions to `model` which are as distant from the optimum and each other using the Random Vector method, but with a maximum `optimality_gap`, using optimisation.
...
function generate_MGA_Rand_Vec!(
    model::JuMP.Model,
    optimality_gap::Float64,
    n_alternatives::Int64;
    ...
    MGA_Rand_Vec_initial!(model, optimality_gap, fixed_variables)
    @info "Solving model."
    JuMP.optimize!(model)
    ...
    for i = 2:n_alternatives
      ...
      MGA_Rand_Vec_alternatives!(model)
      @info "Solving model."
      JuMP.optimize!(model)
...
Transform a JuMP model into a model solving its corresponding Random Vector MGA
...
function MGA_Rand_Vec_initial!(
  ...
  fixed_variables::Vector{VariableRef},
    ...
    random_array = rand(Float64, n) * 2 .- 1
...
"""
    MGA_Rand_Vec_alternatives!(
        model::JuMP.Model,
        metric::Distances.Metric
    )

Add a previously found solution to Random vector problem. Used for iteratively finding multiple alternative solutions.
"""
function MGA_Rand_Vec_alternatives!(model::JuMP.Model)
