...
Generate `n_alternatives` solutions to `model` which are as distant from the optimum and each other using the HSJ method, but with a maximum `optimality_gap`, using optimisation.
...
function generate_MGA_HSJ!(
    ...
    MGA_HSJ_initial!(model, optimality_gap, fixed_variables)
    @info "Solving model."
    JuMP.optimize!(model)
...
Transform a JuMP model into a model solving its corresponding HSJ MGA
...
"""
function MGA_HSJ_alternatives(model::JuMP.Model)
  ...
  @objective(model, Min, non_zero_variables)
end
