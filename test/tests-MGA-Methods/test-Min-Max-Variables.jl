@testset "Test generate alternatives with Min/Max Variables as modeling_method." begin
    ...
    results = NearOptimalAlternatives.generate_alternatives!(model, 0.1, all_variables(model), 1; modeling_method = :Min_Max_Variables)
  ...
  @testset "Test regular run with one alternative with one fixed variable." begin
    ...
    results = NearOptimalAlternatives.generate_alternatives!(model, 0.1, all_variables(model), 1; fixed_variables = [x_2], modeling_method = :Min_Max_Variables)
  ...
  @testset "Test regular run with two alternatives." begin
    ...
    results = NearOptimalAlternatives.generate_alternatives!(model, 0.1, all_variables(model), 2; modeling_method = :Min_Max_Variables)
